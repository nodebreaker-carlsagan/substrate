...
pub fn resource_metrics() {
    ...
    prometheus_gauge!(metrics::RESOURCE_RECEIVE_BYTES => sys.get_network().get_income());
    println!("output data: {} B", sys.get_network().get_outcome());
    prometheus_gauge!(metrics::RESOURCE_SENT_BYTES => sys.get_network().get_outcome());
    ...
    // Prometheus usage
    prometheus_gauge!(metrics::RESOURCE_RAM_USE => sys.get_used_memory());
    prometheus_gauge!(metrics::RESOURCE_SWAP_USE => sys.get_used_swap());
